...
This guide describes how to build devcoind, command-line utilities, and GUI on FreeBSD.
...
### 2. Clone Devcoin Repo
Now that `git` and all the required dependencies are installed, let's clone the Devcoin Core repository to a directory. All build scripts and commands will run from this directory.
``` bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run devcoind or the GUI. To enable legacy wallets, you must install `db5`. To enable [descriptor wallets](https://github.com/devcoin/core/blob/master/doc/descriptors.md), `sqlite3` is required. Skip `db5` if you intend to *exclusively* use descriptor wallets
...
Devcoin Core includes a GUI built with the cross-platform Qt Framework. To compile the GUI, we need to install `qt5`. Skip if you don't intend to use the GUI.
```bash
pkg install qt5
```
###### libqrencode
