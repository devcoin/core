# Fuzzing Devcoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Devcoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/devcoin/core
$ cd devcoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/devcoin/core/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/devcoin/core/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/devcoin/core/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/devcoin/core/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`devcoin-core/qa-assets`](https://github.com/devcoin-core/qa-assets) repo.

To fuzz `process_message` using the [`devcoin-core/qa-assets`](https://github.com/devcoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/devcoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`devcoin-core/qa-assets`](https://github.com/devcoin-core/qa-assets) repo.

Every single pull request submitted against the Devcoin Core repo is automatically tested against all inputs in the [`devcoin-core/qa-assets`](https://github.com/devcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Devcoin Core more robust.
...
with certain assembly code from Devcoin Core's code. See [developer notes on sanitizers](https://github.com/devcoin/core/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Devcoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Devcoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Devcoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Devcoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Devcoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Devcoin
Core without having to write any custom fuzzing harness. The `devcoind` server
...
```sh
$ mkdir devcoin-honggfuzz-p2p/
$ cd devcoin-honggfuzz-p2p/
$ git clone https://github.com/devcoin/core
$ cd devcoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/devcoind.cpp b/src/devcoind.cpp
...
Devcoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/devcoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=devcoin-core).
...
because Devcoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_devcoin-core/latest).
