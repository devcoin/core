Devcoin Core version 0.19.0.1 is now available from:

  <https://devcoin.org/bin/devcoin-core-0.19.0.1/>
...
Devcoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Devcoin Core on unsupported systems.
...
to open the Devcoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Devcoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/devcoin/core/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Devcoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Devcoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `devcoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Devcoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Devcoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Devcoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Devcoin Core,
  ...
  This prevents Devcoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Devcoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Devcoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `devcoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Devcoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Devcoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Devcoin Core will be bound to
  the standard port 52333 even if a different port is configured for
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Devcoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  DVC), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in DVC per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Devcoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Devcoin AEAD (jonasschnelli)
...
- #15639 devcoin-wallet tool: Drop libdevcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as DVC/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in devcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "devcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of devcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from devcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_devcoin to auto generated project files (sipsorcery)
...
- #15773 Add DevcoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_devcoin output (practicalswift)
...
- #15896 feature_filelock, interface_devcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Devcoin Core (luke-jr)
...
- #16277 Suppress output in test_devcoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as DVC/kB (jonatack)
...
- #16489 log: harmonize devcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (devcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example devcoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBDEVCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update devcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in devcoin.conf (fanquake)
...
- #16621 add default devcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the devcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, devcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/devcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/devcoin/devcoin/).
