Devcoin Core version 0.12.0 is now available from:

  <https://devcoin.org/bin/devcoin-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Devcoin Core or other software:
...
pre-0.12 versions of Devcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Devcoin Core version 0.11 or
earlier.
...
[libsecp256k1](https://github.com/devcoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/devcoin/bips/blob/master/bip-0130.mediawiki)
...
Devcoin Core 0.12 will have a strict maximum size on the mempool. The
...
Devcoin Core 0.12 also introduces new default policy limits on the length and
...
Devcoin Core 0.12 nodes. Devcoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/devcoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Devcoin Core 0.12 does not yet have support for
...
In Devcoin Core 0.12, when mempool limit has been reached a higher minimum
...
notes of v0.11.0](https://github.com/devcoin/core/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/devcoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/devcoin/bips/blob/master/bip-0065.mediawiki)
...
- #6214 `87406aa` [OSX] revert renaming of Devcoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new devcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include devcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix devcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of devcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override DEVCOIND and DEVCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [devcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add DEVCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libdevcoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "DVC" (Ross Nicoll)
