Devcoin Core version 0.16.0 is now available from:

  <https://devcoin.org/bin/devcoin-core-0.16.0/>
...
Devcoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
devcoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Devcoin Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µDVC" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- devcoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- devcoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.devcoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for devcoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to devcoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µDVC to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_devcoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` devcoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.devcoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for devcoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in devcoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop devcoind (OmeGak)
...
- #10529 `331352f` Improve devcoind systemd service file (Flowdalic)
