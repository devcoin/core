...
Devcoin Core version 0.20.0 is now available from:

  <https://devcoin.org/bin/devcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `DEVCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Devcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Devcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Devcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Devcoin Core. Devcoin Core logs debug messages
...
- OpenSSL is no longer used by Devcoin Core.  (#17265)

- BIP70 support has been fully removed from Devcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/devcoin/core/issues/16599),
  [PR #16702](https://github.com/devcoin/core/pull/16702), and the
  `devcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Devcoin Core starts are now
...
- The "Start Devcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://devcoin.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Devcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Devcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/devcoin-core/HWI) for
  signing.  Future versions of Devcoin Core should support a GUI option
...
- Devcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.devcoin.org.  (#17596)
...
- The `devcoin-cli` utility used with the `-getinfo` parameter now
...
- The `devcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `devcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_devcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBDEVCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_devcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBDEVCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for devcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate devcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest devcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_devcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (devcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone devcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Devcoin Core files (hebasto)
...
- #17119 Fix broken devcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from devcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.devcoin.org (laanwj)
...
- #17648 Rename wallet-tool references to devcoin-wallet (hel-o)
...
- #18341 Replace remaining literal DVC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On devcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from devcoin-cli -getinfo (malevolent)
