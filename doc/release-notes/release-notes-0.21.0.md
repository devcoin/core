...
Devcoin Core version 0.21.0 is now available from:

  <https://devcoin.org/bin/devcoin-core-0.21.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Devcoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Devcoin Core on
unsupported systems.

From Devcoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Devcoin Core does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/devcoin/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Devcoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Devcoin Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/devcoin/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/devcoin/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/devcoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/devcoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/devcoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `devcoin-cli -netinfo` command provides a network peer connections
...
- A new `devcoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `devcoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `devcoin-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Devcoin Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `devcoin-wallet` tool which performs the salvage
...
Devcoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`devcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Devcoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `devcoin-cli createwallet` or `devcoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/devcoin/core/issues).
...
Users who already have a Devcoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Devcoin Core.
...
The change to using descriptors changes the default derivation paths used by Devcoin Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in DVC per 1,000
  vbytes (DVC/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from DVC/kB
  to sat/vB and the help documentation in DVC/kB is updated to DVC/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from DVC/kvB to sat/vB and the
  ...
  difference between DVC/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in DVC/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Devcoin Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `devcoin.conf`
...
- #19284 Add seed.devcoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in devcoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add devcoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in devcoin-cli (laanwj)
...
### Build system
- #18504 Drop devcoin-tx and devcoin-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in DEVCOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for devcoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Devcoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitredevcoin)
...
- #18817 Document differences in devcoind and devcoin-qt locale handling (practicalswift)
