Devcoin Core version 0.14.0 is now available from:

  <https://devcoin.org/bin/devcoin-core-0.14.0/>
...
Devcoin Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/devcoin/core/blob/master/doc/zmq.md) for
...
When Devcoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `devcoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`devcoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/devcoin-cli -named help command="help"
    src/devcoin-cli -named getblockhash height=0
    src/devcoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/devcoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://devcoin.org/en/alert/2016-11-01-alert-retirement) in Devcoin Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Devcoin Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/devcoin/core/pull/8753)
...
- #8722 `bb843ad` devcoin-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run devcoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to devcoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to devcoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and devcoin-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_devcoin_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change devcoin address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` devcoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow devcoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` devcoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to devcoin-tx (jnewbery)
