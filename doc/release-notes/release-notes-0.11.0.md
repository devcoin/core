Devcoin Core version 0.11.0 is now available from:

  <https://devcoin.org/bin/devcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer DVC/kB fee to be
...
For example, add the following to `devcoin.conf`:
...
related to the blockchain in the devcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Devcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Devcoin Core will 
...
For an example script see [devcoin-submittx](https://github.com/laanwj/devcoin-submittx).
...
- #5334 `cf87536` libdevcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new devcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only DevcoinAmountField
- #5753 `0f44672` Add devcoin logo to about screen
...
- #5809 `46bfbe7` Add devcoin-cli man page
...
- #5849 `41259ca` contrib/init/devcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Devcoin-Qt.app to Devcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for devcoin-tx)
