...
This guide describes how to build devcoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Devcoin Core from source.
...
### 4. Clone Devcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Devcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `devcoind` or  `devcoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/devcoin/core/blob/master/doc/descriptors.md), `sqlite` is required.
...
Devcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/devcoin/core/issues/7714).
...
You can deploy a `.dmg` containing the Devcoin Core application using `make deploy`.
...
There are many ways to configure Devcoin Core, here are a few common examples:
...
Run the following in your terminal to compile Devcoin Core:
...
## Running Devcoin Core

Devcoin Core should now be available at `./src/devcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/devcoin-qt`.

The first time you run `devcoind` or `devcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Devcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Devcoin"

touch "/Users/${USER}/Library/Application Support/Devcoin/devcoin.conf"
...
./src/qt/devcoin-qt -server # Starts the devcoin-qt server mode, allows devcoin-cli control
```
