...
**Devcoin Core's `configure` script by default will ignore the depends output.** In
...
RISC-V known issue: gcc-7.3.0 and gcc-7.3.1 result in a broken `test_devcoin` executable (see https://github.com/devcoin/core/pull/13543),
...
options will be passed to devcoin's configure. In this case, `--disable-wallet`.
