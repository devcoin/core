...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<devcoin-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<devcoin-cli args>]`

* `--cmd` lets you customize the devcoin-cli path. By default it will look for it in the PATH
* `--faucet` lets you specify which faucet to use; the faucet is assumed to be compatible with https://github.com/kallewoof/devcoin-faucet
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `devcoin-cli` if you use `--addr`)
...
To mine the first block in your custom chain, you can run:
    ...
    CLI="./devcoin-cli -conf=mysignet.conf"
