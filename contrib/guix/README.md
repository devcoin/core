# Bootstrappable Devcoin Core Builds

This directory contains the files necessary to perform bootstrappable Devcoin
Core builds.
...
To build Devcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Devcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/devcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/devcoin-core/guix.sigs).
