...
make -C depends --jobs="$JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform devcoin-linux-g++' \
                                   qt_config_opts_x86_64_linux='-platform linux-g++ -xplatform devcoin-linux-g++' \
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} DEVCOIN_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Devcoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Devcoin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
