...
# Be aware that devcoind and devcoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in devcoind.
#
...
# C locale.
#
# In contrast, devcoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where devcoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of devcoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in devcoind but
# "1,230000" in devcoin-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/devcoin-tx.cpp.*stoul"
    "src/devcoin-tx.cpp.*trim_right"
