...
def check_json_precision():
    """Make sure json library being used does not lose precision converting DVC values"""
...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `DevcoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "devcoin.conf"), n=n, chain=chain)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "devcoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "devcoin.conf")):
        with open(os.path.join(datadir, "devcoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
