...
Verify that a devcoind node can load multiple wallet files
"""
...
class MultiWalletTest(DevcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            exp_stderr = r"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another devcoind?"
        else:
        ...
        if self.options.descriptors:
            assert_raises_rpc_error(-4, "Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another devcoind?", self.nodes[0].loadwallet, wallet_names[0])
        else:
