...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The CI linter job also checks this, but [possibly not in all cases](https://github.com/devcoin/core/pull/14884#discussion_r239585126).
...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the DevcoinTestFramework, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 DVC) in its wallet. Using them is much more
...
with the devcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a devcoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Devcoin Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the DevcoinTestFramework
...
Taken from the [python-devcoinrpc repository](https://github.com/jgarzik/python-devcoinrpc).
...
Test objects for interacting with a devcoind node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-devcoinlib)
