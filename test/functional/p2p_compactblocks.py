...
# TestP2PConn: A peer we use to send messages to devcoind, and store responses.
...
class CompactBlocksTest(DevcoinTestFramework):
    ...
    # This test actually causes devcoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that devcoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for devcoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # devcoind will not send blocktxn responses for blocks whose height is
