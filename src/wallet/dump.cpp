...
static const std::string DUMP_MAGIC = "DEVCOIN_CORE_WALLET_DUMP";
...
// queue, which doesn't exist for the devcoin-wallet. Define our own
// deleter here.
...
bool CreateFromDump(const std::string& name, const fs::path& wallet_path, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (ver != DUMP_VERSION) {
        error = strprintf(_("Error: Dumpfile version is not supported. This version of devcoin-wallet only supports version 1 dumpfiles. Got dumpfile with version %s"), version_value);
        dump_file.close();
        return false;
    }
