...
static bool WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s devcoind-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";
        if (!args.IsArgSet("-version")) {
            strUsage += "\n"
                        "devcoind-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
                        "By default devcoind-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n"
                        "To change the target wallet, use the -datadir, -wallet and -testnet/-regtest arguments.\n\n"
                        "Usage:\n"
                        "  devcoind-wallet [options] <command>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
int main(int argc, char* argv[])
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `devcoind-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
