...
void PeerManagerImpl::FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller)
{
    ...
    if (state->pindexBestKnownBlock != nullptr) {
        std::cout << "DEBUG: peer ChainWork " << state->pindexBestKnownBlock->nChainWork.GetHex().c_str() << std::endl << std::endl;
    }
    ...
    while (!orphan_work_set.empty()) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            if (state.GetResult() != TxValidationResult::TX_WITNESS_STRIPPED) {
                ...
                // https://github.com/devcoin/core/issues/8279 for details.
                // We can remove this restriction (and always add wtxids to
                // the filter even for witness stripped transactions) once
                // wtxid-based relay is broadly deployed.
                // See also comments in https://github.com/devcoin/core/pull/18044#discussion_r443419034
