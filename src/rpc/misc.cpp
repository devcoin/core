...
static RPCHelpMan validateaddress()
{
    return RPCHelpMan{"validateaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The devcoin address to validate"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
...
static RPCHelpMan verifymessage()
{
    return RPCHelpMan{"verifymessage",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The devcoin address to use for the signature."},
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "devcoin-cli and the GUI. There is no server-side difference.",
                {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = Assert(EnsureAnyNodeContext(request.context).init)->ipc()) {
                // Spawn a new devcoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new devcoin-echo executable,
                // and spawn devcoin-echo below instead of devcoin-node. But
                // using devcoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("devcoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a devcoind
                // process not a devcoind-node process, so just create a local
