...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/devcoin.conf are parsed in qt/devcoin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (!args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo()) + "\n"
                "\nUsage:  devcoind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see devcoind -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for devcoind but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect devcoind signal handlers
    noui_connect();
