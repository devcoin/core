...
#include <qt/devcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new DevcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new DevcoinAddressCheckValidator(parent));
}
...
bool parseDevcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no devcoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!DevcoinUnits::parse(DevcoinUnits::DVC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseDevcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseDevcoinURI(uriInstance, out);
}

QString formatDevcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("devcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DevcoinUnits::format(DevcoinUnits::DVC, info.amount, false, DevcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openDevcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", DEVCOIN_CONF_FILENAME));
    ...
    /* Open devcoin.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("devcoin-%s.desktop", chain);
}
