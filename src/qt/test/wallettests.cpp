// Copyright (c) 2015-2020 The Bitcoin Core and Devcoin Core developers
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<DevcoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_devcoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_devcoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_devcoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = DevcoinUnits::formatWithUnit(unit, balance, false, DevcoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    DevcoinAmountField* amountInput = receiveCoinsDialog.findChild<DevcoinAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_devcoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
