This directory contains the source code for the Devcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Devcoin UI.
...
- ClientModel is used by the main application `devcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a devcoin:... link from a browser).
...
* UI elements like DevcoinAmountField, which inherit from QWidget.
* `devcoinstrings.cpp`: automatically generated
* `devcoinunits.(h/cpp)`: DVC / mDVC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/devcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `devcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `devcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `devcoin-qt`
9. You're all set! Start developing, building, and debugging the Devcoin Core GUI
