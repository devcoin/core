dnl Copyright (c) 2013-2016 The Bitcoin Core and Devcoin Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set devcoin_enable_qt to false. Else, exit.
AC_DEFUN([DEVCOIN_QT_FAIL],[
  if test "x$devcoin_qt_want_version" = xauto && test "x$devcoin_qt_force" != xyes; then
    if test "x$devcoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; devcoin-qt frontend will not be built])
    fi
    devcoin_enable_qt=no
    devcoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([DEVCOIN_QT_CHECK],[
  if test "x$devcoin_enable_qt" != xno && test "x$devcoin_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl DEVCOIN_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([DEVCOIN_QT_PATH_PROGS],[
  DEVCOIN_QT_CHECK([
    if test "x$3" != x; then
      AC_PATH_PROGS($1,$2,,$3)
    else
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      DEVCOIN_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other DEVCOIN_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([DEVCOIN_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build devcoin-qt GUI (default=auto)])],
    [
     devcoin_qt_want_version=$withval
     if test "x$devcoin_qt_want_version" = xyes; then
       devcoin_qt_force=yes
       devcoin_qt_want_version=auto
     fi
    ],
    [devcoin_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); devcoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   DEVCOIN_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _DEVCOIN_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: devcoin_enable_qt, devcoin_enable_qt_dbus, devcoin_enable_qt_test
AC_DEFUN([DEVCOIN_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  DEVCOIN_QT_CHECK([_DEVCOIN_QT_FIND_LIBS])
  ...
  dnl the final binary as well. _DEVCOIN_QT_CHECK_STATIC_PLUGIN does
  ...
  _DEVCOIN_QT_IS_STATIC
  if test "x$devcoin_cv_static_qt" = xyes; then
    _DEVCOIN_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _DEVCOIN_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _DEVCOIN_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      _DEVCOIN_QT_CHECK_STATIC_PLUGIN([QWindowsVistaStylePlugin], [-lqwindowsvistastyle])
      ...
      _DEVCOIN_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _DEVCOIN_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _DEVCOIN_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  DEVCOIN_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  DEVCOIN_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  DEVCOIN_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  DEVCOIN_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  DEVCOIN_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  DEVCOIN_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  DEVCOIN_QT_CHECK([
    devcoin_enable_qt=yes
    devcoin_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      devcoin_enable_qt_test=no
    fi
    devcoin_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      devcoin_enable_qt_dbus=yes
    fi
  ...
  if test x$devcoin_enable_qt = xyes; then
    AC_MSG_RESULT([$devcoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$devcoin_enable_qt])
  fi
...
dnl
dnl _DEVCOIN_QT_IS_STATIC
dnl ---------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
dnl Output: devcoin_cv_static_qt=yes|no
AC_DEFUN([_DEVCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, devcoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [devcoin_cv_static_qt=yes],
      [devcoin_cv_static_qt=no])
    ])
])

dnl Internal. Check if the link-requirements for a static plugin are met.
dnl
dnl _DEVCOIN_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_DEVCOIN_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); DEVCOIN_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _DEVCOIN_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_DEVCOIN_QT_CHECK_STATIC_LIBS], [
...
dnl
dnl _DEVCOIN_QT_FIND_LIBS
dnl ---------------------
dnl
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_DEVCOIN_QT_FIND_LIBS],[
  DEVCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [DEVCOIN_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  DEVCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [DEVCOIN_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  DEVCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [DEVCOIN_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  DEVCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [DEVCOIN_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
